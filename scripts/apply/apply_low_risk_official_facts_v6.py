...
REPORT_MD_PATH = "output/reports/low_risk_source_v6_official_facts_apply_report.md"

# Expected from V5 dry-run
EXPECTED_UPDATED_ROWS = 24
EXPECTED_REGION_UPDATES = 19
EXPECTED_CASK_UPDATES = 15
EXPECTED_SOURCE_REFS = 34
...
def main():
    parser = argparse.ArgumentParser(description="Guarded apply for official facts batch 2.")
    parser.add_argument('--apply', action='store_true')
    parser.add_argument('--confirm', type=str, default='')
    args = parser.parse_args()

    is_dry_run = not args.apply
    expected_phrase = "WRITE GO: apply low risk official facts batch 2 to production.db"
    ...
    backup_path = f"output/import/production_before_low_risk_official_facts_v6_{timestamp}.db"

    # Choose target DB
    if is_dry_run:
        target_db = "output/tmp/low_risk_source_v6_dry_run_temp.db"
    ...
    # Ensure source references table exists
        ...
        for c in update_candidates:
            ...
            for field, val in updates.items():
                if field not in ('region', 'cask_type'):
                    continue  # Only these two fields in this batch
    ...
    report = []
    report.append("# Low-Risk Official Facts Batch 2 Guarded Apply Report\n")
