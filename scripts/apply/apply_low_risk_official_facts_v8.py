...
REPORT_MD_PATH = "output/reports/low_risk_source_v8_official_facts_apply_report.md"

# Expected from V7 dry-run
EXPECTED_UPDATED_ROWS = 25
EXPECTED_REGION_UPDATES = 18
EXPECTED_CASK_UPDATES = 15
EXPECTED_SOURCE_REFS = 33
...
def main():
    parser = argparse.ArgumentParser(description="Guarded apply for official facts batch 3.")
    parser.add_argument('--apply', action='store_true')
    parser.add_argument('--confirm', type=str, default='')
    args = parser.parse_args()

    is_dry_run = not args.apply
    expected_phrase = "WRITE GO: apply low risk official facts batch 3 to production.db"
    ...
    backup_path = f"output/import/production_before_low_risk_official_facts_v8_{timestamp}.db"

    if is_dry_run:
        target_db = "output/tmp/low_risk_source_v8_dry_run_temp.db"
    ...
    try:
        ...
        # Validation against V7 dry-run expectations
    ...
    report = []
    report.append("# Low-Risk Official Facts Batch 3 Guarded Apply Report\n")
    ...
    if not is_dry_run and execution_status.startswith("Success"):
        ...
        report.append("*(Already applied)*")
    else:
        report.append("\n## Required Confirm Phrase (for Real Apply)")
        report.append(f'`python scripts/apply/apply_low_risk_official_facts_v8.py --apply --confirm "{expected_phrase}"`')
