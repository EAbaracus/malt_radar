...
# Previous files for exclusion checks
EXCLUSION_FILES = [
    "data/output/low_risk_source_v2_official_facts_fetch_candidates.csv",
    "data/output/low_risk_source_v3_official_facts_update_plan.csv",
    "data/output/low_risk_source_v5_official_facts_fetch_candidates.csv",
    "data/output/low_risk_source_v5_official_facts_update_plan.csv",
    "data/output/low_risk_source_v7_official_facts_fetch_candidates.csv",
    "data/output/low_risk_source_v7_official_facts_update_plan.csv",
    "data/output/low_risk_source_v9_official_facts_fetch_candidates.csv",
    "data/output/low_risk_source_v9_official_facts_update_plan.csv"
]

OUTPUT_DIR = "data/output"
FETCH_CANDIDATES_CSV = os.path.join(OUTPUT_DIR, "low_risk_source_v11_official_facts_fetch_candidates.csv")
READY_CSV = os.path.join(OUTPUT_DIR, "low_risk_source_v11_official_facts_ready_candidates.csv")
UPDATE_PLAN_CSV = os.path.join(OUTPUT_DIR, "low_risk_source_v11_official_facts_update_plan.csv")
SOURCE_REFS_PLAN_CSV = os.path.join(OUTPUT_DIR, "low_risk_source_v11_official_source_references_plan.csv")
MANUAL_REVIEW_CSV = os.path.join(OUTPUT_DIR, "low_risk_source_v11_official_facts_manual_review.csv")
BLOCKED_CSV = os.path.join(OUTPUT_DIR, "low_risk_source_v11_official_facts_blocked.csv")
REPORT_MD = "output/reports/low_risk_source_v11_official_facts_batch_report.md"

BATCH_OFFSET = 550
BATCH_LIMIT = 200

# Dictionary of mapped distillery domains and their regions
DISTILLERY_DOMAINS = {
    ...
    'Midleton': ('midletonveryrare.com', 'Irish'),
    'Green Spot': ('spotwhiskey.com', 'Irish'),
    'Yellow Spot': ('spotwhiskey.com', 'Irish'),
    'Tullamore': ('tullamoredew.com', 'Irish'),
    'Teeling': ('teelingwhiskey.com', 'Irish'),
    'Dingle': ('dingledistillery.ie', 'Irish'),
    'Waterford': ('waterfordwhisky.com', 'Irish'),
    'Buffalo Trace': ('buffalotracedistillery.com', 'American'),
    'Makers Mark': ('makersmark.com', 'American'),
    'Woodford Reserve': ('woodfordreserve.com', 'American'),
    'Wild Turkey': ('wildturkey.com', 'American'),
    'Elijah Craig': ('heavenhill.com', 'American'),
    'Four Roses': ('fourroses.us', 'American'),
}
...
def extract_cask(name):
    m = re.search(
        r'\b(sherry|bourbon|port|wine|oak|cask|barrel|butt|puncheon|hogshead|manzanilla|oloroso|pedro ximenez|px|rum|cognac|mizunara|virgin|ex-bourbon|ex-sherry|peated|first fill|double cask|triple cask)\b',
...
def main():
    ...
    # Load previously processed whisky_id + field combos
    processed_combos = set()
    for plan_csv in EXCLUSION_FILES:
        if os.path.exists(plan_csv):
            with open(plan_csv, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    ...
                    if not wid:
                        continue
                    if 'updates_to_apply' in fields:
                        try:
                            updates = json.loads(row.get('updates_to_apply', '{}'))
                            for field in updates:
                                processed_combos.add(f"{wid}|{field}")
                        except:
                            pass
                    else:
                        # V2 fallback: if status is ready, read what was extracted
                        status = row.get('candidate_status', '')
                        if 'ready' in status.lower():
                            if not is_empty(row.get('extracted_age')):
                                processed_combos.add(f"{wid}|age")
                            if not is_empty(row.get('extracted_abv')):
                                processed_combos.add(f"{wid}|abv")
                            if not is_empty(row.get('extracted_region')):
                                processed_combos.add(f"{wid}|region")
                            if not is_empty(row.get('extracted_cask_type')):
                                processed_combos.add(f"{wid}|cask_type")

    all_queue = []
    if os.path.exists(OFFICIAL_QUEUE_CSV):
    ...
    for c in batch:
        ...
        if extracted_age and f"{wid}|age" not in processed_combos and is_empty(w_db.get('age')):
            field_updates['age'] = extracted_age

        if extracted_abv and f"{wid}|abv" not in processed_combos and is_empty(w_db.get('abv')):
            try:
        ...
        if extracted_region and f"{wid}|region" not in processed_combos and is_empty(w_db.get('region')):
            field_updates['region'] = extracted_region

        if extracted_cask and f"{wid}|cask_type" not in processed_combos and is_empty(w_db.get('cask_type')):
    ...
    def write_csv(path, data, fields=None):
        with open(path, 'w', newline='', encoding='utf-8') as f:
            if data:
                ...
                fnames = fields or ['whisky_id', 'candidate_status']
    ...
    # Ensure source refs fields match and file exists
    ref_fields = ['whisky_id', 'whisky_name', 'proposed_official_url', 'proposed_source_domain', 'field_name', 'field_value', 'confidence', 'license_risk', 'copyright_risk']
    write_csv(SOURCE_REFS_PLAN_CSV, source_refs_plan, ref_fields)

    # Write Batch Report
    remaining_in_queue = max(0, len(all_queue) - (BATCH_OFFSET + len(batch)))
    
    report = []
    report.append("# Low-Risk Official Facts Batch V11 Report\n")
    ...
    report.append(f"\n## P1 Queue Remaining")
    report.append(f"- Remaining candidates after batch: {remaining_in_queue}")

    report.append("\n## Final GO/NO-GO")
    report.append("**GO** (Batch V11 official facts candidate pack built successfully).")

    report.append("\n## Next Phase Recommendation")
    if remaining_in_queue == 0:
        report.append("- **WHISKEYMAPPER-RECON-V1**: P1 queue is now completely exhausted. Proceeding to WhiskeyMapper reconciliation.")
    else:
        report.append("- **LOW-RISK-SOURCE-V12**: Continue processing remaining P1 queue or perform guarded apply.")
