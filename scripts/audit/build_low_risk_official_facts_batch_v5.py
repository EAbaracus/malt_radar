...
PREV_UPDATE_PLAN_CSV = "data/output/low_risk_source_v3_official_facts_update_plan.csv"

OUTPUT_DIR = "data/output"
FETCH_CANDIDATES_CSV = os.path.join(OUTPUT_DIR, "low_risk_source_v5_official_facts_fetch_candidates.csv")
READY_CSV = os.path.join(OUTPUT_DIR, "low_risk_source_v5_official_facts_ready_candidates.csv")
UPDATE_PLAN_CSV = os.path.join(OUTPUT_DIR, "low_risk_source_v5_official_facts_update_plan.csv")
SOURCE_REFS_PLAN_CSV = os.path.join(OUTPUT_DIR, "low_risk_source_v5_official_source_references_plan.csv")
MANUAL_REVIEW_CSV = os.path.join(OUTPUT_DIR, "low_risk_source_v5_official_facts_manual_review.csv")
BLOCKED_CSV = os.path.join(OUTPUT_DIR, "low_risk_source_v5_official_facts_blocked.csv")
REPORT_MD = "output/reports/low_risk_source_v5_official_facts_batch_report.md"
...
def extract_cask(name):
    m = re.search(r'\b(sherry|bourbon|port|wine|oak|cask|barrel|butt|puncheon|hogshead|manzanilla|oloroso|pedro ximenez|px|rum|cognac|mizunara)\b',
...
def main():
    ...
    # Load previously processed whisky_id + field combos (V3)
    processed_combos = set()
    if os.path.exists(PREV_UPDATE_PLAN_CSV):
        with open(PREV_UPDATE_PLAN_CSV, 'r', encoding='utf-8') as f:
            for row in csv.DictReader(f):
                updates = json.loads(row.get('updates_to_apply', '{}'))
                for field in updates:
                    processed_combos.add(f"{row['whisky_id']}|{field}")

    # Load queue batch (offset 100, limit 150 → indices 100–249)
    all_queue = []
    ...
    FIELDS_SCHEMA = ['whisky_id', 'whisky_name', 'distillery_name', 'current_age', 'current_abv',
    ...
    for c in batch:
        ...
        extracted_region = domain_info[1] if domain_info else (region if region != 'Unknown' else None)
        ...
        # Evaluate which fields can be updated
        field_updates = {}

        if extracted_age and not is_empty(w_db.get('age')):
            pass  # already filled
        elif extracted_age and f"{wid}|age" not in processed_combos:
            if is_empty(w_db.get('age')):
                field_updates['age'] = extracted_age

        if extracted_abv and not is_empty(w_db.get('abv')):
            pass
        elif extracted_abv and f"{wid}|abv" not in processed_combos:
            if is_empty(w_db.get('abv')):
                try:
        ...
        if extracted_region and not is_empty(w_db.get('region')):
            pass
        elif extracted_region and f"{wid}|region" not in processed_combos:
            if is_empty(w_db.get('region')):
                field_updates['region'] = extracted_region

        if extracted_cask and not is_empty(w_db.get('cask_type')):
            pass
        elif extracted_cask and f"{wid}|cask_type" not in processed_combos:
            if is_empty(w_db.get('cask_type')):
        ...
        if not domain:
            ...
            row['reason'] = 'Official domain known but no extractable metadata found'
            ...
            row['reason'] = 'All extractable metadata already present in DB or previously processed'
            stats['manual_review'] += 1
            manual_list.append(row)
    ...
    # Write report
    report = []
    report.append("# Low-Risk Official Facts Batch V5 Report\n")
    ...
    report.append("- **100% Compliant**: Only factual parameters extracted. No tasting notes, no review prose.")

    report.append("\n## Final GO/NO-GO")
    report.append("**GO** (Batch V5 official facts candidate pack built successfully).")

    report.append("\n## Next Phase")
    report.append("- **LOW-RISK-SOURCE-V6 — Guarded Apply Batch 2**: Apply the update_candidate rows and source references to production.")
