...
class DbReadService:
    ...
    def _prepare_chip_filter(self, filter: str):
        ...
        static_conds = {
            ...
            "blended": "(LOWER(w.type) IN ('blend','blended') OR LOWER(w.category) IN ('blended','blend'))",
            "bourbon": "(LOWER(w.category) = 'bourbon' OR LOWER(w.type) = 'bourbon')",
            "rye": "(LOWER(w.category) = 'rye' OR LOWER(w.type) = 'rye')"
        }
